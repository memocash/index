...
type TxLostLoader struct {
	...
	cache map[string]*model.TxLost

	// the current batch. keys will continue to be collected until timeout is hit,
	// then everything will be sent to the fetch method and out to the listeners
	batch *txLostLoaderBatch

	// mutex to prevent races
	mu sync.Mutex
}

type txLostLoaderBatch struct {
	keys    []string
	data    []*model.TxLost
	error   []error
	closing bool
...
// LoadThunk returns a function that when called will block waiting for a TxLost.
...
func (l *TxLostLoader) LoadThunk(key string) func() (*model.TxLost, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*model.TxLost, error) {
			return it, nil
		}
	}
	if l.batch == nil {
		l.batch = &txLostLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *TxLostLoader) LoadAll(keys []string) ([]*model.TxLost, []error) {
	results := make([]func() (*model.TxLost, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	txLosts := make([]*model.TxLost, len(keys))
	errors := make([]error, len(keys))
	for i, thunk := range results {
		txLosts[i], errors[i] = thunk()
	}
	return txLosts, errors
}

// LoadAllThunk returns a function that when called will block waiting for a TxLosts.
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
...
func (l *TxLostLoader) unsafeSet(key string, value *model.TxLost) {
	if l.cache == nil {
		l.cache = map[string]*model.TxLost{}
	}
	l.cache[key] = value
}
...
func (b *txLostLoaderBatch) keyIndex(l *TxLostLoader, key string) int {
...
func (b *txLostLoaderBatch) startTimer(l *TxLostLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *txLostLoaderBatch) end(l *TxLostLoader) {
