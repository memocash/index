...
type TxSeenLoader struct {
	...
	cache map[string]*model.Date

	// the current batch. keys will continue to be collected until timeout is hit,
	// then everything will be sent to the fetch method and out to the listeners
	batch *txSeenLoaderBatch

	// mutex to prevent races
	mu sync.Mutex
}

type txSeenLoaderBatch struct {
	keys    []string
	data    []*model.Date
	error   []error
	closing bool
...
// LoadThunk returns a function that when called will block waiting for a Date.
...
func (l *TxSeenLoader) LoadThunk(key string) func() (*model.Date, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*model.Date, error) {
			return it, nil
		}
	}
	if l.batch == nil {
		l.batch = &txSeenLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *TxSeenLoader) LoadAll(keys []string) ([]*model.Date, []error) {
	results := make([]func() (*model.Date, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	dates := make([]*model.Date, len(keys))
	errors := make([]error, len(keys))
	for i, thunk := range results {
		dates[i], errors[i] = thunk()
	}
	return dates, errors
}

// LoadAllThunk returns a function that when called will block waiting for a Dates.
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
...
func (l *TxSeenLoader) unsafeSet(key string, value *model.Date) {
	if l.cache == nil {
		l.cache = map[string]*model.Date{}
	}
	l.cache[key] = value
}
...
func (b *txSeenLoaderBatch) keyIndex(l *TxSeenLoader, key string) int {
...
func (b *txSeenLoaderBatch) startTimer(l *TxSeenLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *txSeenLoaderBatch) end(l *TxSeenLoader) {
