...
type TxSuspectLoader struct {
	...
	cache map[string]*model.TxSuspect

	// the current batch. keys will continue to be collected until timeout is hit,
	// then everything will be sent to the fetch method and out to the listeners
	batch *txSuspectLoaderBatch

	// mutex to prevent races
	mu sync.Mutex
}

type txSuspectLoaderBatch struct {
	keys    []string
	data    []*model.TxSuspect
	error   []error
	closing bool
	done    chan struct{}
}
...
// LoadThunk returns a function that when called will block waiting for a TxSuspect.
...
func (l *TxSuspectLoader) LoadThunk(key string) func() (*model.TxSuspect, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*model.TxSuspect, error) {
			return it, nil
		}
	}
	if l.batch == nil {
		l.batch = &txSuspectLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *TxSuspectLoader) LoadAll(keys []string) ([]*model.TxSuspect, []error) {
	results := make([]func() (*model.TxSuspect, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	txSuspects := make([]*model.TxSuspect, len(keys))
	errors := make([]error, len(keys))
	for i, thunk := range results {
		txSuspects[i], errors[i] = thunk()
	}
	return txSuspects, errors
}

// LoadAllThunk returns a function that when called will block waiting for a TxSuspects.
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
...
func (l *TxSuspectLoader) unsafeSet(key string, value *model.TxSuspect) {
	if l.cache == nil {
		l.cache = map[string]*model.TxSuspect{}
	}
	l.cache[key] = value
}
...
func (b *txSuspectLoaderBatch) keyIndex(l *TxSuspectLoader, key string) int {
...
func (b *txSuspectLoaderBatch) startTimer(l *TxSuspectLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *txSuspectLoaderBatch) end(l *TxSuspectLoader) {
