...
func (l *BlockLoader) LoadAll(keys []string) ([][]*model.Block, []error) {
	results := make([]func() ([]*model.Block, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	blocks := make([][]*model.Block, len(keys))
...
func (l *BlockLoader) Prime(key string, value []*model.Block) bool {
	...
	if _, found = l.cache[key]; !found {
		...
		cpy := make([]*model.Block, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (l *BlockLoader) unsafeSet(key string, value []*model.Block) {
	if l.cache == nil {
		l.cache = map[string][]*model.Block{}
	}
	l.cache[key] = value
}
