...
// Tx is the resolver for the tx field.
func (r *subscriptionResolver) Tx(ctx context.Context, hash model.Hash) (<-chan *model.Tx, error) {
	OpenSubscriptionWithRequest(ctx, "tx")
	ctx, cancel := context.WithCancel(ctx)
	txProcessedListener, err := chain.ListenTxProcessed(ctx, [][32]byte{hash})
	if err != nil {
		cancel()
	...
	go func() {
		...
		for {
			...
			var tx = &model.Tx{Hash: hash}
			if err := attach.ToTxs(ctx, fields, []*model.Tx{tx}); err != nil {
				log.Printf("error attaching to txs for tx subscription; %v", err)
				return
			}
			txChan <- tx
		}
