...
func (o *SlpOutputs) AttachValidities() {
	defer o.Wait.Done()
	if !o.HasField([]string{"validity"}) {
		return
	}
	o.Mutex.Lock()
	var txHashes = make([][32]byte, 0, len(o.SlpOutputs))
	for i := range o.SlpOutputs {
		o.SlpOutputs[i].Validity = model.SlpValidityPending
		txHashes = append(txHashes, o.SlpOutputs[i].Hash)
	}
	o.Mutex.Unlock()
	validities, err := slp.GetValidities(o.Ctx, txHashes)
	if err != nil {
		o.AddError(fmt.Errorf("error getting slp validities for attach to slp outputs; %w", err))
		return
	}
	...
	for i := range o.SlpOutputs {
		if validity, ok := validityMap[[32]byte(o.SlpOutputs[i].Hash)]; ok {
			if validity.IsValid() {
				o.SlpOutputs[i].Validity = model.SlpValidityValid
			} else {
				o.SlpOutputs[i].Validity = model.SlpValidityInvalid
			}
		}
	}
}
...
func (o *SlpOutputs) getTokenHashIndexMap() map[[32]byte][]int {
	o.Mutex.Lock()
	defer o.Mutex.Unlock()
	m := make(map[[32]byte][]int, len(o.SlpOutputs))
	for i := range o.SlpOutputs {
		m[o.SlpOutputs[i].TokenHash] = append(m[o.SlpOutputs[i].TokenHash], i)
	}
	return m
}

func (o *SlpOutputs) getSlpOutputIndexMap() map[outKey][]int {
	o.Mutex.Lock()
	defer o.Mutex.Unlock()
	m := make(map[outKey][]int, len(o.SlpOutputs))
	for i := range o.SlpOutputs {
		k := outKey{o.SlpOutputs[i].Hash, o.SlpOutputs[i].Index}
		m[k] = append(m[k], i)
	}
	return m
}
...
func (o *SlpOutputs) AttachOutputs() {
	...
	slpOutputIndexMap := o.getSlpOutputIndexMap()
	var allOutputs []*model.TxOutput
	o.Mutex.Lock()
	for j := range txOutputs {
		indices, ok := slpOutputIndexMap[outKey{txOutputs[j].TxHash, txOutputs[j].Index}]
		if !ok {
			continue
		}
		for _, i := range indices {
